...
\fancyhead[R]{\small\sffamily\color{Navy}项目技术报告 V1.4}
...
\newcommand{\DatasetHash}{\texttt{b66f4f367b93ef6f\ldots 8f1d37}}
\newcommand{\BestModel}{Global FNO-L}
\newcommand{\BestRMSE}{0.703}
\newcommand{\BestMAE}{0.261}
\newcommand{\BestPninety}{0.922}
\newcommand{\BestWorst}{1.129}
\newcommand{\GlobalBaseline}{5.625}
\newcommand{\RelativeReduction}{87.49}
\newcommand{\GpuPninetyfive}{4.47}
\newcommand{\CpuPninetyfive}{133.72}
\newcommand{\BellhopMedian}{8.00}
...
TEOS-10 转换为声速；只叠加幅度受限的四参数平滑扰动。正式数据扩展至 256 个环境样本，
按地形分层划分为 192/32/32 个训练、验证和独立测试样本。

参考标签由 Bellhop 非相干场生成。8 场逐级射线数审计表明，12,800 到 25,600 射线的
平均差为 0.230 dB、最差样本为 0.286 dB，因此正式数据固定使用 25,600 条射线。方法上，
在二维 FNO 的基础上引入显式地形通道、各向异性谱截断、非周期复制填充、层间残差连接
和全局训练均值残差参数化。模型学习 SSP 与地形共同决定的、相对于全局训练均值场的变化。
...
6 月窄域 SSP 族与经筛选的低维 GEBCO 地形剖面。橙色星号为剖面起点。}
...
\end{equation}
四维参数采用固定种子的分块嵌套 Latin hypercube 采样。前 128 个设计点保持冻结，扩容时
增加一个独立的 128 点空间填充块，因此旧标签可以逐项校验后直接复用。该参数化只产生
平滑剖面，不制造网格尺度随机噪声。
...
运行 3200、6400、12,800 和 25,600 条射线，并以相邻级别场差评估数值稳定性。此次扩容
没有改变频率、网格、地形、SSP 范围或 Bellhop 参数，因此沿用同一冻结收敛审计，不重复
消耗计算资源生成等价的射线阶梯标签。
...
正式数据集包含 256 个样本，每条地形对应 64 条独立 SSP。每个地形内部固定划分为
48 个训练、8 个验证和 8 个测试样本，汇总为 192/32/32。划分单位是完整环境样本，绝不
把同一二维场的网格点随机分散到多个 split。测试标签在模型结构和超参数确定后才用于
一次性评分。扩容过程逐元素核验并复用原有 128 个高射线标签，仅为新增 SSP 设计点补算
128 个 Bellhop 场；合并后重新执行按地形分层的固定划分。
...
$h=1,\ldots,H$。本轮最终设置为
\begin{equation}
  N=256,\qquad N_s=41,\qquad N_z=96,\qquad N_r=256,\qquad H=32.
\end{equation}
...
\begin{table}[H]
\centering
\caption{最终 Global FNO-L 的逐模块输入、输出和作用}
...
这里的“改进”不是简单增大网络宽度：相对于基础 FNO，本模型显式接收距离相关海底边界，
在两个空间方向采用不同截断模态数 $(K_z,K_r)=(16,48)$，对非周期网格执行复制 padding，
并在每个谱模块外增加局部路径和层间残差。四项设计分别对应“环境边界可见”“距离方向保留
更多变化”“降低边界绕回”和“保留未截断的局部信息/稳定深层训练”。

\subsection{一个二维 Fourier 神经算子模块如何计算}
...
\figfull{0.99}{fig04_model_architecture.pdf}{最终 Global FNO-L 的端到端张量维度，以及单个
...
decay $10^{-5}$、cosine 调度、batch size 16，最多 220 epochs。每个 epoch 后把验证预测
...
48/96/192 场训练 & 同一 Global FNO-S、同一密封测试集 & 数据量学习曲线 \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
全局训练均值场 & -- & \GlobalBaseline & 3.857 & 6.386 & -- \\
Global FNO-S & 1.33 & 0.843 & 0.383 & 1.122 & 3.93 \\
Global FNO-L & 6.30 & \BestRMSE & \BestMAE & \BestPninety & \GpuPninetyfive \\
\bottomrule
\end{tabular}
\end{table}

\figfull{0.98}{fig05_main_results.pdf}{全局均值场与两个神经算子规模的测试 RMSE，
以及各模型相对全局均值场的 RMSE 降幅。}

\subsection{数据量消融}

在不改变验证集和测试集的条件下，从每类地形的 48 个训练样本中固定抽取 12、24、48 个，
对应总训练量 48、96、192。全局均值场和残差尺度也只由各轮实际训练样本计算。该实验直接衡量
数据量收益，不混入额外地形或测试样本。

\figfull{0.72}{fig06_data_size_ablation.pdf}{同一 Global FNO-S 在 48、96 和 192 个训练场下的
学习曲线。横向比较使用完全相同的密封测试集。}

\subsection{优化过程与典型场}

\figfull{0.78}{fig07_training_curves.pdf}{两个主要 FNO 规模的验证 RMSE 轨迹。模型选择仅依赖
验证集，红色虚线为 2 dB 门槛。}
...
Global FNO-S，其本次 CPU P95 为 50.11 ms、RMSE 为 0.843 dB。25,600 射线 Bellhop
单场中位耗时为 \BellhopMedian{} s。Bellhop 时间只用于说明标签生成成本，不将不同软件
栈的单次运行简单等同为严格加速比。

\figfull{0.78}{fig10_latency.pdf}{两个 FNO 规模的 batch=1 P95 时延与 25,600 射线 Bellhop
单场中位耗时。纵轴采用对数尺度。}
...
全局均值场和残差尺度只由 192 个训练样本计算；验证和测试标签不参与统计量或网络参数
...
\begin{enumerate}
  ...
  \item 基于 GEBCO 2026 与 WOA23 构造了 256 场小而有区分度的受控数据集；
  \item 通过 25,600 射线收敛审计保证当前 reference 的数值稳定性；
  \item 提出显式地形编码、全局残差参数化、非周期 padding 和各向异性 FNO 组合方法；
...
数据集 ID & \path{bashi_gebco_selected_diverse_woa23_june_v0.6_n256} \\
样本数与划分 & 256；train/validation/test = 192/32/32 \\
输出网格 & 96 depth $\times$ 256 range \\
正式射线数 & 25,600/field \\
数据 SHA-256 & \path{b66f4f367b93ef6f84802604e1d7f25e85d1b6a20a224252e6776b1285ef1d37} \\
最终模型 & \BestModel \\
代码版本 & Git tag \path{technical-report-v1.4}，具体 SHA 见仓库 release \\
...
# 2. Train the frozen final experiment from an existing n256 dataset.
...
脚本默认使用本报告的 realistic config、256 样本和最终实验；也可通过
...
experiment = next(
    ...
    if item["id"] == "real_r2_terrain_fno"
)
dataset = cfg.dataset_root / "n256/dataset.npz"
