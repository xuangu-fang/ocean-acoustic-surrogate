...
\hypersetup{
  ...
  urlcolor=Blue,
  citecolor=Teal,
  pdftitle={多声源深度条件下的深海声传播损失快速计算技术报告},
...
\fancyhead[R]{\small\sffamily\color{Navy}项目技术报告 V1.6}
...
\newcommand{\DatasetHash}{\texttt{7be279cbbaf5bd09\ldots 2eadfc1}}
\newcommand{\BestModel}{SourceBAR-FNO}
\newcommand{\BestRMSE}{1.241}
\newcommand{\BestMAE}{0.622}
\newcommand{\BestPninety}{1.736}
\newcommand{\BestWorst}{2.616}
\newcommand{\GlobalBaseline}{4.223}
\newcommand{\RelativeReduction}{70.62}
\newcommand{\GpuPninetyfive}{4.30}
\newcommand{\CpuPninetyfive}{141.64}
\newcommand{\BellhopMedian}{7.80}
...
GEBCO 地形、WOA23 月度声速剖面与 SourceBAR-FNO}\par
...
\textbf{核心参数} & 1 kHz、50 km、50--1000 m 六档声源深度、96$\times$256 输出网格 \\
...
本项目面向频率 1 kHz、最大传播距离 50 km 的典型深海声传播损失快速计算任务，研究从
声速剖面、距离相关海底地形及\textbf{显式声源深度}到二维非相干传播损失（Transmission
Loss, TL）场的代理映射。声源深度覆盖 50、100、200、400、700 和 1000 m 六档。
核心验收要求为：相对于 Bellhop 数值参考，独立测试集总体 RMSE 和 MAE 均不超过 2 dB；
A100 上单场模型计算 P95 不超过 100 ms。
...
幅度受限的四参数平滑扰动。正式数据集为
$4$ 类地形$\times3$ 个月份$\times6$ 档声源深度$\times6$ 个扰动，共 432 个完整声场；
按三因素组合严格分层为 288/72/72 个训练、验证和独立测试样本。

参考标签由 Bellhop 非相干场生成。覆盖六档源深的 12 场审计表明，12,800 到 25,600
射线的平均差为 0.219 dB、最差为 0.310 dB，因此正式数据固定使用 25,600 条射线。
方法上提出 Source-conditioned Bathymetry-Aware Residual Fourier Neural Operator
（SourceBAR-FNO）：通过 SSP--地形--源深三条件编码、各向异性谱传播核、全局--局部
残差融合、非周期边界保护及训练域残差解码，使一个模型处理多种声源深度。
...
完整推理 P95 为 \GpuPninetyfive{} ms，满足 100 ms 指标。六档声源深度分组 RMSE 为
1.080--1.477 dB，全部低于 2 dB；相对于不提供源深输入的同结构模型（3.122 dB），
显式源深条件使误差下降约 60.2\%。本报告中的“参考场”均指公开
...
给定一维声速剖面 $c(z)$、距离相关海底深度 $b(r)$、声源深度 $z_s$，以及固定频率和
海底介质参数，
项目需要构造代理算子
\begin{equation}
  \mathcal{G}_\theta:\big(c(z),b(r),z_s\big)\longmapsto \TL(z,r;z_s),
\end{equation}
...
\midrule
声源频率 & 1000 Hz & 单频 \\
声源深度 & 50、100、200、400、700、1000 m & 每个样本显式给定一个标量 \\
...
真实数据锚定环境中的准确率和时延闭环。测试样本与训练样本共享四类地形、三个月份和
六档离散源深定义，但 SSP 扰动参数独立；因此结果衡量的是受控组合域内的插值能力，
不把离散档位之间任意连续源深或未见地形外推作为本轮结论。
...
3/6/12 月 SSP 族、经筛选的低维 GEBCO 地形剖面及六档声源深度。橙色星号为剖面起点。}
...
\end{equation}
四维参数采用固定种子的 Latin hypercube 采样。正式因子设计为 4 类地形、3 个真实月份、
6 档源深及每个组合 6 个独立扰动，共 72 个三因素组合、432 场。所得参数实际范围分别为
$[-0.550,0.547]$、$[-0.798,0.799]$ m/s、$[-34.85,34.84]$ m 和
$[-0.550,0.547]$ m/s。该参数化只产生平滑剖面，不制造网格尺度随机噪声；同一月份内部
保持适度变化，把本轮新增难度集中在可解释的声源深度维度。

\subsection{Bellhop 高质量标签}

每个环境样本生成一个 96$\times$256 的非相干 TL 场。海底采用声速 1700 m/s、密度
2000 kg/m$^3$、衰减 0.8 dB/$\lambda$ 的流体半空间。正式计算前，在六档源深上各抽取
两个代表环境，共 12 场，在 12,800 和 25,600 条射线下重新运行，并以场差评估数值稳定性。
...
\midrule
12,800 $\rightarrow$ 25,600 & 0.219 & 0.310 & 0.178 \\
\bottomrule
\end{tabular}
\end{table}
...
正式数据集包含 432 个样本，即
$4$ 类地形$\times3$ 个月份$\times6$ 档源深$\times6$ 个独立 SSP 扰动。
每个地形--月份--源深组合固定划分为 4 个训练、1 个验证和 1 个测试样本，汇总为
288/72/72；因此每档源深在 train/validation/test 中分别有 48/12/12 场。
划分单位是完整环境样本，绝不把同一二维场的网格点随机分散到多个 split。测试标签在模型
结构和超参数确定后才用于一次性评分。扩容过程逐元素核验并复用与上一冻结版本严格匹配的
72 个 50 m 高射线标签，为其余源深和设计点新计算 360 个 Bellhop 场；总成功率 432/432。

每个样本保存 TL、有效掩膜、SSP、地形、\texttt{source\_depths\_m}、坐标、扰动参数和
Bellhop case 元数据。打包文件
...
\section{SourceBAR-FNO：源深、季节与地形感知的残差 Fourier 神经算子}

本项目将最终方法命名为 \textbf{SourceBAR-FNO}（Source-conditioned Bathymetry-Aware Residual
Fourier Neural Operator）。名称对应三项核心设计：Source-conditioned 表示声源深度作为
可见条件直接进入计算图；Bathymetry-Aware 表示距离相关海底边界显式进入网络；Residual
表示模型预测训练均值场之上的变化，而不是从零拟合全部 TL。月份差异由 SSP 数值本身表达。
该名称是对实际计算图的概括，
并不额外引入无法复核的物理先验。
...
$h=1,\ldots,H$。本轮最终设置为
\begin{equation}
  N=432,\qquad N_s=41,\qquad N_z=96,\qquad N_r=256,\qquad H=32.
\end{equation}
...
\subsubsection{声源深度的显式条件编码}

声源深度不是从输出中反推，也不是按深度训练六个互不相干的模型，而是作为每个样本的
已知条件输入。推荐模型采用归一化标量平面
\begin{equation}
  S_{i,1,p,q}=\frac{z_{s,i}}{2000},\qquad
  S_i\in\mathbb{R}^{1\times96\times256}.
\end{equation}
对同一样本，$S_i$ 在空间上为常数；但逐点升维与后续谱卷积会把该条件与 SSP、地形和
坐标共同混合，从而学习 $\TL(z,r;z_s)$ 对源深的依赖。消融另测试了宽度 75 m 的局部高斯
标记 $\exp[-(z_p-z_s)^2/(2\cdot75^2)]$，其精度未优于标量平面，因此未纳入推荐配置。

三个物理输入最终按通道拼接为
\begin{equation}
  X_i=\operatorname{Concat}_{C}(C_i,B_i,S_i)
  \in\mathbb{R}^{3\times96\times256}.
\end{equation}
最终模型不使用 Hankel 人工特征；输入信息仅来自 SSP、地形、声源深度及随后加入的坐标。
...
\end{equation}
并把 $[X_i,Z,R]$ 拼成 $5\times96\times256$。逐点 $1\times1$ 卷积把每个
位置的 5 维向量映射到 $H=32$ 维隐藏表示：
\begin{equation}
  V_0=\operatorname{GELU}\big(P[X_i,Z,R]+p_0\big),\qquad
  V_0\in\mathbb{R}^{32\times96\times256},\quad P\in\mathbb{R}^{32\times5}.
\end{equation}
...
\begin{table}[H]
\centering
\caption{最终 SourceBAR-FNO 的逐模块输入、输出和作用}
\label{tab:tensor-contract}
\begin{tabularx}{\textwidth}{L{2.7cm}L{3.1cm}L{3.1cm}Y}
\toprule
模块 & 输入维度 & 输出维度 & 具体作用 \\
\midrule
物理特征组装 & SSP $[B,41]$；地形 $[B,256]$；源深 $[B,1]$ & $[B,3,96,256]$ &
插值、标准化，并按各变量定义复制到二维网格 \\
坐标拼接 & $[B,3,96,256]$ & $[B,5,96,256]$ &
加入深度坐标 $Z$ 和距离坐标 $R$，让算子区分绝对位置 \\
$1\times1$ 升维 & $[B,5,96,256]$ & $[B,32,96,256]$ &
每个网格点独立将 5 个输入量映射为 32 个隐特征 \\
...
SourceBAR-FNO 可分为六个可独立说明的模块：
\begin{enumerate}
  \item \textbf{显式源深条件编码（Explicit Source Conditioning）}：将物理源深归一化并
  复制为全场条件，使同一组 SSP 与地形在不同 $z_s$ 下仍可辨识；
...
\figfull{0.99}{fig04_model_architecture.pdf}{最终 SourceBAR-FNO 的端到端张量维度，以及单个
...
decay $10^{-5}$、cosine 调度、batch size 16，最多 300 epochs。每个 epoch 后把验证预测
解码回 dB 域计算完整 RMSE；只保存验证 RMSE 最低的 checkpoint，测试集不参与早停或选模。

部署时一个样本依次执行：（1）读取 41 点 SSP、256 点地形和一个源深标量；（2）形成
$[1,3,96,256]$ 物理输入；（3）网络内部加入坐标并升维；（4）经过 padding 和四个 FNO block；
...
无源深编码 & SSP、地形及结构不变，但移除 $z_s$ & 检验多源深任务是否必须显式可辨 \\
SourceBAR-FNO & 加入归一化源深标量；$H=32$ & 推荐的精度--规模--时延折中 \\
标量 + 局部源标记 & 再加入 75 m 高斯深度标记；$H=32$ & 检验局部几何提示是否提供额外信息 \\
宽 SourceBAR-FNO & 标量 + 局部标记；$H=48$ & 检验剩余误差是否受模型容量限制 \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
全局训练均值场 & -- & \GlobalBaseline & 2.796 & 6.426 & -- \\
无源深编码 & 6.30 & 3.122 & 1.931 & 4.623 & 4.60 \\
\textbf{SourceBAR-FNO} & \textbf{6.30} & \textbf{\BestRMSE} & \textbf{\BestMAE} &
  \textbf{\BestPninety} & \textbf{\GpuPninetyfive} \\
标量 + 局部源标记 & 6.30 & 1.249 & 0.611 & 1.730 & 5.65 \\
宽 SourceBAR-FNO & 14.17 & 1.236 & 0.603 & 1.714 & 6.05 \\
\bottomrule
\end{tabular}
\end{table}

\figfull{0.98}{fig05_main_results.pdf}{全局均值场、无源深编码、推荐模型及两个结构变体的测试
RMSE，以及相对全局均值场的降幅。}

\subsection{模块消融分析}

移除源深输入后，模型面对相同 SSP 和地形却对应六个不同声场，测试 RMSE 为 3.122 dB，
未通过 2 dB 门槛。加入单一归一化源深标量后，RMSE 降至 1.241 dB，相对该消融下降
60.2\%，说明\textbf{显式源深条件是本轮决定性增量}。继续加入局部高斯源标记后 RMSE 为
1.249 dB，没有形成额外收益；这表明当前六档离散深度已能由全局标量与坐标通道联合表达。
将隐藏宽度从 32 增至 48 后，RMSE 仅由 1.241 降至 1.236 dB，但参数量由 6.30 M 增至
14.17 M。0.005 dB 的差异不足以抵消 2.25 倍参数开销，故推荐紧凑标量版本。

\subsection{按声源深度、月份与地形分组结果}

六档源深 50/100/200/400/700/1000 m 的分组 RMSE 分别为
1.261、1.080、1.477、1.130、1.212 和 1.246 dB，全部通过 2 dB 门槛。三个月份分组
RMSE 为 1.147--1.343 dB；四类地形分组为 0.890--1.537 dB。所有统计均使用同一个冻结
模型和同一密封测试集，不为某个源深、月份或地形单独选模。

\figfull{0.99}{fig06_stratified_results.pdf}{SourceBAR-FNO 在 WOA23 3/6/12 月、四类 GEBCO
地形和六档声源深度上的密封测试 RMSE；所有子组均通过 2 dB 门槛。}
...
\CpuPninetyfive{} ms，因此正式时延验收平台为 A100。25,600 射线 Bellhop
单场中位耗时为 \BellhopMedian{} s。Bellhop 时间只用于说明标签生成成本，不将不同软件
栈的单次运行简单等同为严格加速比。

独立进程重新加载推荐 checkpoint 后复算测试 RMSE 为 1.240752 dB，与训练记录
1.240743 dB 相差 $9.4\times10^{-6}$ dB；数据 SHA、设备类型和误差容差检查均通过。

\figfull{0.78}{fig10_latency.pdf}{三个显式源深 FNO 变体的 batch=1 P95 时延与 25,600 射线 Bellhop
单场中位耗时。纵轴采用对数尺度。}
...
同理，均值场不按源深查表；测试时声源深度只通过输入条件通道进入模型。

\subsection{结论边界}

本报告可支持的结论限定为：在四条经筛选的巴士海峡 GEBCO 形态、WOA23 3/6/12 月 SSP、
1 kHz、六档 50--1000 m 源深、50 km 和固定底质的仿真域内，模型能够复现当前 Bellhop
标签并满足精度及时延指标。测试覆盖已知月份家族、地形类别和离散源深档位内的独立扰动，
不等同于对未见月份或任意连续源深的外推。当前不宣称跨海区、未见地形类别、多频率、
任意连续源深、任意底质或海试
...
\begin{enumerate}
  \item 固定 1 kHz、50 km、六档 50--1000 m 源深和二维输出网格，明确了可检验任务；
  \item 基于 GEBCO 2026、WOA23 三个代表月份和六档源深构造了 432 场受控数据集；
  \item 通过 25,600 射线收敛审计保证当前 reference 的数值稳定性；
  \item 提出 SourceBAR-FNO，将显式源深条件、双环境编码、各向异性谱核、全局--局部
  残差融合、边界保护和训练域残差解码组合为一条可复核计算链；
...
\midrule
\endhead
数据配置 & \path{configs/multi_source_depth_mvp.yaml} \\
实验配置 & \path{configs/multi_source_depth_campaign.yaml} \\
数据集 ID & \path{bashi_gebco_four_terrain_woa23_m03_m06_m12_z0050_z1000_v0.8_n432} \\
样本数与划分 & 432；train/validation/test = 288/72/72 \\
声源深度 & 50、100、200、400、700、1000 m \\
输出网格 & 96 depth $\times$ 256 range \\
正式射线数 & 25,600/field \\
数据 SHA-256 & \path{7be279cbbaf5bd09549abf70a1814d4530fe53ca6c3a45764df5e07bd2eadfc1} \\
最终模型 & \BestModel \\
代码版本 & Git tag \path{technical-report-v1.6}，具体 SHA 见仓库 release \\
...
# 2. Train SourceBAR-FNO from an existing n432 dataset.
...
脚本默认使用本报告的 multi-source-depth config、432 样本和推荐 SourceBAR-FNO 实验；也可通过
...
uv run ocean-acoustic-surrogate verify \
  configs/multi_source_depth_mvp.yaml "$REPRO_RUN_DIR" \
  --samples 432 --device auto
\end{lstlisting}
...
文件 & 职责 \\
\midrule
\path{configs/multi_source_depth_mvp.yaml} & 环境、月份、源深、地形、SSP、Bellhop、split 和验收门槛 \\
\path{configs/multi_source_depth_campaign.yaml} & SourceBAR-FNO、训练超参数和源深编码消融实验 \\
...
\path{src/ocean_acoustic_surrogate/features.py} & SSP/地形/源深输入和全局残差变换 \\
...
cfg = MVPConfig.from_yaml("configs/multi_source_depth_mvp.yaml")
...
cfg = MVPConfig.from_yaml("configs/multi_source_depth_mvp.yaml")
campaign = load_campaign("configs/multi_source_depth_campaign.yaml")
experiment = next(
    item for item in campaign["experiments"]
    if item["id"] == "sourcebar_a1_scalar_source"
)
dataset = cfg.dataset_root / "n432/dataset.npz"
