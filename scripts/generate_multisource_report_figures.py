"""Generate client-facing figures for the multi-source-depth V1.6 report."""
...
CONFIG_PATH = ROOT / "configs/multi_source_depth_mvp.yaml"
GEBCO_PATH = Path(
...
PILOT_DATASET_ID = (
    "bashi_gebco_four_terrain_woa23_m03_m06_m12_z0050_z1000_v0.8_n432"
)
RECOMMENDED_EXPERIMENT = "sourcebar_a1_scalar_source"
BLUE = "#2f6f9f"
NAVY = "#153f66"
TEAL = "#2a8c82"
GREEN = "#4d8f62"
...
def _load() -> tuple[MVPConfig, dict[str, np.ndarray], dict, dict, list[dict]]:
    ...
    dataset_path = ARTIFACT_ROOT / "datasets" / config.contract.dataset_id / "n432/dataset.npz"
    ...
    recommended = next(
        record
        for record in campaign["experiments"]
        if record["experiment_id"] == RECOMMENDED_EXPERIMENT
    )
    campaign = {**campaign, "best": recommended}
...
def plot_pipeline() -> None:
    ...
    boxes = [
        ...
        (0.22, "Controlled domain", "4 terrain × 3 months × 6 depths\n432 balanced fields", "#edf6f4"),
        (0.42, "Reference labels", "Bellhop incoherent TL\n25,600 rays / field", "#fff3e6"),
        (0.62, "SourceBAR-FNO", "source-conditioned environment\n+ anisotropic residual operator", "#eaf3eb"),
...
def plot_method() -> None:
    ...
    axis.axis("off")
    top = [
        (0.015, 0.13, "Raw conditions", "SSP [B, 41]; terrain [B, 256]\nsource depth [B, 1]", "#e8f1f8"),
        ...
        (0.655, 0.13, "4 SourceBAR blocks", "spectral + local + skip\n[B, 32, 104, 272]", "#eaf3eb"),
    ...
    axis.text(
        0.50,
        0.52,
        "One anisotropic SourceBAR block (H=32, Kz=16, Kr=48); global and local updates retain the padded shape",
        ha="center",
        va="center",
        fontsize=9,
        color=NAVY,
...
def plot_environment(config: MVPConfig, data: dict[str, np.ndarray]) -> None:
    fig, axes = plt.subplots(1, 4, figsize=(20.0, 4.7), constrained_layout=True)
    ...
    source_depths = np.unique(data["source_depths_m"])
    axes[3].scatter(
        np.arange(len(source_depths)),
        source_depths,
        s=95,
        color=ORANGE,
        edgecolor="white",
    ...
    axes[3].vlines(
        np.arange(len(source_depths)), 0, source_depths, color=BLUE, alpha=0.35
    )
    axes[3].set_xticks(
        np.arange(len(source_depths)), [f"{value:g}" for value in source_depths]
    )
    axes[3].invert_yaxis()
    axes[3].grid(alpha=0.18)
    axes[3].set(
        xlabel="Source-depth setting (m)",
        ylabel="Depth below surface (m)",
        title="Six explicitly modeled source depths",
    )
...
def _run_label(experiment_id: str) -> str:
    labels = {
        "sourcebar_a0_no_source_encoding": "No source depth",
        "sourcebar_a1_scalar_source": "SourceBAR-FNO",
        "sourcebar_fno": "Scalar + source marker",
        "sourcebar_fno_h48": "Wide H=48",
    }
...
def plot_stratified_results(campaign: dict) -> None:
    ...
    source = metrics["metrics"]["test"]["by_source_depth"]
    ...
    source_order = ["z0050m", "z0100m", "z0200m", "z0400m", "z0700m", "z1000m"]
    bars = axes[2].bar(
        ["50", "100", "200", "400", "700", "1000"],
        [source[key]["rmse_db"] for key in source_order],
        color=[BLUE, TEAL, GREEN, ORANGE, RED, NAVY],
    )
    axes[2].bar_label(bars, fmt="%.3f", fontsize=8)
    axes[2].set(
        xlabel="Source depth (m)",
        ylabel="Held-out RMSE (dB)",
        title="Performance by source depth",
    )
    for axis in axes:
...
def plot_prediction_analysis(campaign: dict) -> None:
    ...
    with np.load(run_dir / "predictions.npz") as raw:
        ...
        source_depths = raw["source_depths_m"]
...
def plot_latency(runs: list[dict], pilot: dict) -> None:
    main = [
        run
        for run in runs
        if run["metrics"]["experiment_id"] != "sourcebar_a0_no_source_encoding"
    ]
